...
# ── Legacy HTML routes (kept for Jinja2 views) ────────────────────────────────
...
def hub():
    wards = Ward.query.order_by(Ward.code).all()
    return render_template("governance/hub.html", wards=wards)


@bp.get("/html/funds")
def funds():
    funds_q = GovernmentFund.query.all()
    projects = PublicProject.query.all()
    fund_by_project = {f.project_id: f for f in funds_q}
    rows = [(p, fund_by_project.get(p.id)) for p in projects]
    return render_template("governance/funds.html", rows=rows)


@bp.get("/html/projects")
def projects():
    items = PublicProject.query.order_by(PublicProject.id.desc()).limit(50).all()
    return render_template("governance/projects.html", projects=items)


@bp.get("/html/contact/<int:ward_id>")
def contact(ward_id: int):
    ward = Ward.query.get_or_404(ward_id)
    oc = OfficeContact.query.filter_by(ward_id=ward.id).first()
    return render_template("governance/contact.html", ward=ward, contact=oc)


@bp.route("/html/complaints/new", methods=["GET", "POST"])
@login_required
def new_complaint():
    wards = Ward.query.order_by(Ward.code).all()
    if request.method == "POST":
        ...
        ward_id = int(request.form.get("ward_id") or 0)
        is_anon = request.form.get("anonymous") == "on"
        if not title or not body or not ward_id:
            flash("Title, description and ward are required.", "error")
            return render_template("governance/complaint_form.html", wards=wards)
        c = Complaint(
            user_id=None if is_anon else g.current_user.id,
            ward_id=ward_id,
            title=title,
            body=body,
            is_anonymous=is_anon,
        ...
        db.session.add(c)
        log_audit(g.current_user.id, "complaint_create", f"complaint:{c.title[:40]}")
        db.session.commit()
...
@bp.get("/html/wards/<int:ward_id>/dashboard")
def ward_dashboard_html(ward_id: int):
    ward = Ward.query.get_or_404(ward_id)
    alerts = (AcquisitionAlert.query.filter_by(ward_id=ward.id)
              .order_by(AcquisitionAlert.created_at.desc()).limit(10).all())
    complaints = (Complaint.query.filter_by(ward_id=ward.id)
                  .order_by(Complaint.created_at.desc()).limit(20).all())
    return render_template("governance/ward_dashboard.html", ward=ward, alerts=alerts, complaints=complaints)


@bp.get("/html/emergency")
def emergency_html():
    return render_template("governance/emergency.html")
