...
def login():
    if request.method == "POST":
        ...
        log_audit(user.id, "web_login", "user", email)
        db.session.commit()
        return resp
...
def register():
    if request.method == "POST":
        ...
        role_raw = (request.form.get("role") or "citizen").lower()
        if not email or not password or not full_name:
        ...
        try:
            ...
            role = UserRole.CITIZEN
        if role == UserRole.ADMIN:
            return render_template("auth/register.html", error="Admin accounts are provisioned separately.")
        if User.query.filter_by(email=email).first():
